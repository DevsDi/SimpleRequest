...
// AbortController of the current request (used to cancel the request)
  ...
  (message: ExecuteRequestMessage, _sender, sendResponse) => {
    ...
    if (message.type === 'cancelRequest') {
      ...
      if (currentAbortController) {
        ...
        // Mark as manually cancelled (to distinguish from timeout)
...
function buildHeaders(request: HttpRequestInternal, hasFormDataBody: boolean): Record<string, string> {
  ...
  // The Content-Type of a form-data request must be auto-generated by fetch (including the boundary);
  // a manual value (e.g. multipart/form-data without a boundary) would prevent the server from parsing the body, so it is always stripped
  if (hasFormDataBody) {
...
function parseFormDataContent(content: string): FormData {
  ...
  for (const line of lines) {
    ...
    // Tighten the file detection: only parse as a file entry when "=@" is followed by ";type=" or ";base64,";
    // otherwise a text value that happens to contain "=@" (e.g. a signature string) would be misidentified as a file entry
...
async function executeRequest(request: HttpRequestInternal): Promise<HttpResponse> {
  ...
  // Cancel the previous request (ensure only one request runs at a time)
  if (currentAbortController) {
    currentAbortController.abort();
  }
 ...
 * Network errors, timeouts, and 5xx status codes can be retried
 */
...
async function executeRequestWithRetry(request: HttpRequestInternal): Promise<HttpResponse> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // The last attempt failed; throw the error directly
      if (attempt === maxRetries) {
        throw error;
      }

      // Only retry on network errors, timeouts, etc.
  ...
  // This branch should never be reached, but TypeScript requires a return value
